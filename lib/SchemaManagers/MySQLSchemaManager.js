 ...
 *
 * @returns {Promise} function resolve({Object<Table>} tables)
 */
p.listTables = function() {
  var that = this;

  return MySQLSchemaManager.super_.prototype.listTables.call(this).then(function(tables) {
    return that._conn.getDatabase().then(function(database) {
      var sql =
        'SELECT t.`table_name`, t.`engine`, t.`table_collation`, t.`table_comment`, ' +
        '       t.`auto_increment`, t.`row_format`, ccsa.`character_set_name` ' +
        'FROM INFORMATION_SCHEMA.`TABLES` t ' +
        'LEFT JOIN INFORMATION_SCHEMA.`COLLATION_CHARACTER_SET_APPLICABILITY` ccsa ON t.`TABLE_COLLATION` = ccsa.`COLLATION_NAME`' +
        'WHERE `table_schema` = ' + that._platform.quoteStringLiteral(database) + ' AND ' +
        "      `table_type` = 'BASE TABLE' "
      ;

      return that._conn.fetchAll(sql).then(function(rows) {
        each(rows, function(row) {
          var options = tables[row['table_name']].getOptions();

          options['engine'] = row['engine'];
          options['collate'] = row['table_collation'];
          options['auto_increment'] = row['auto_increment'];
          options['comment'] = row['table_comment'];
          options['row_format'] = row['row_format'];
          options['charset'] = row['character_set_name'];
        });
